...
for(const width of [390,1440]) {
 ...
 for(const [role,name,home] of roles){
  await page.goto(origin+'/app/sign-in');await page.getByRole('button',{name:role,exact:true}).click();await page.getByRole('button',{name:new RegExp(name)}).click();await page.waitForURL('**'+home)
