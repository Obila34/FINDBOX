  ...
  return <div className="fb-auth-page">
    ...
    <main className="fb-auth-main fb-container">
      <section className="fb-auth-story">
        <div className="fb-studio-backdrop" aria-hidden="true"><div className="fb-fluted-art" /></div>
      ...
      <section className={cx('fb-auth-form fb-enter', width === 'md' && 'fb-auth-form-wide')}>
        {['/app/sign-in', '/app/sign-up'].includes(pathname) ? <nav className="fb-auth-tabs" aria-label="Account access"><Link to="/app/sign-in" aria-current={pathname === '/app/sign-in' ? 'page' : undefined}>Sign in</Link><Link to="/app/sign-up" aria-current={pathname === '/app/sign-up' ? 'page' : undefined}>Sign up</Link></nav> : <p className="fb-auth-kicker">{tone === 'school' ? 'School workspace' : 'Your FindBox'}</p>}
